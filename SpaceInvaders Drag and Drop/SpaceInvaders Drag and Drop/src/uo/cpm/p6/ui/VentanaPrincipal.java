...
public class VentanaPrincipal extends JFrame {
	...
	private ProcesaLabelTablero pBT = new ProcesaLabelTablero();
	...
	private JLabel nuevaLabel (int i) {
		//Sustituimos al actionCommand por el texto de la Label, ya que las labels no lo tienen
		...
		labelTablero.setTransferHandler(new TransferHandler("icon"));
		//PASO 7 DRAG AND DROP
		labelTablero.addPropertyChangeListener(pBT);
		return labelTablero;
	}
	...
	private void compruebaFin() {
		if (spaceInvaders.isPartidaFinalizada()) {
			...
			descubreTablero();
			//PASO 8: ELIMINAMOS LOS DISPAROS NO GASTADOS PARA NO PODER SEGUIR DISPARANDO
	...
	private JLabel nuevoDisparo() {
		...
		lbDisparo.setTransferHandler(new TransferHandler("icon"));
		return lbDisparo;
	}
	...
	//PASO 6 DRAG AND DROP. Sustituimos el ActionListener por un PropertyChangeListener
	class ProcesaLabelTablero implements PropertyChangeListener {
		public void propertyChange(PropertyChangeEvent e) {
			if (e.getPropertyName().equals("icon")  && (e.getNewValue() != null) && (e.getNewValue().toString().contains("shoot.png"))) {
				...
				lbPulsada.setTransferHandler(null);  // para evitar soltar sobre una casilla que ya está ocupada
	...
	private void inicializar() {
		...
		// PASO 9 DRAG AND DROP: Para iniciar un nuevo juego
		despintaTablero();
		
		//ALTERNATIVA PASO 9: creamos de nuevo las labels pq varias de las anteriores ya tienen la propiedad icon cambiada
	...
	private void despintaTablero() {
		for (int i=0;i<getPnTablero().getComponents().length; i++) {
			...
			//Volver a poner transfer ya que lo habíamos quitado para no poder arrastrar a la misma etiqueta
