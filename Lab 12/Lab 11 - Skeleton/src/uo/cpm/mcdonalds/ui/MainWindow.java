...
public class MainWindow extends JFrame {
	...
	private JPanel getPn1() {
		if (pn1 == null) {
			...
			pn1.add(getPnFilter_1(), BorderLayout.WEST);
	...
	private JList getFoodList() {
		if (foodList == null) {
			foodList = new JList();
			foodList.addKeyListener(new KeyAdapter() {
				@Override
				public void keyPressed(KeyEvent e) {
					if (e.getKeyCode()==KeyEvent.VK_DELETE) {
						Product product = (Product) foodList.getSelectedValue();
						foodModel.removeElement(product);
						mcDonalds.remove(product,1);
						getTxtPrice().setText("Price: "+String.format("%.2f", mcDonalds.getOrderTotal()));
					}
				}
			});
	...
	private JList getDrinksList() {
		if (drinksList == null) {
			...
			drinksList.addKeyListener(new KeyAdapter() {
				@Override
				public void keyPressed(KeyEvent e) {
					if (e.getKeyCode()==KeyEvent.VK_DELETE) {
						Product product = (Product) drinksList.getSelectedValue();
						drinksModel.removeElement(product);
						mcDonalds.remove(product,1);
						getTxtPrice().setText("Price: "+String.format("%.2f", mcDonalds.getOrderTotal()));
					}
				}
	...
	private JPanel getPnFilter_1() {
		if (pnFilter == null) {
			pnFilter = new JPanel();
			pnFilter.setLayout(new GridLayout(0, 1, 0, 0));
			pnFilter.add(getBtnBurguer());
	...
	private JButton getBtnBurguer() {
		if (btnBurguer == null) {
			btnBurguer = new JButton("Burguers");
			btnBurguer.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					List<Product> products = mcDonalds.getMenuProducts();
					for(int i=0; i<products.size();i++) {
						if (!products.get(i).getType().equals("Burger")) {
							getPnProducts().getComponent(i).setEnabled(false);
						}
						else {
							getPnProducts().getComponent(i).setEnabled(true);
						}
			...
			btnBurguer.setHorizontalTextPosition(SwingConstants.CENTER);
			btnBurguer.setVerticalTextPosition(SwingConstants.BOTTOM);
			btnBurguer.setIcon(new ImageIcon(MainWindow.class.getResource("/img/filter/Hamburguesa.png")));
			btnBurguer.setMnemonic('B');
		}
		return btnBurguer;
	}
		...
		if (btnDrinks == null) {
			...
			btnDrinks.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					List<Product> products = mcDonalds.getMenuProducts();
					for(int i=0; i<products.size();i++) {
						if (!products.get(i).getType().equals("Drink")) {
							getPnProducts().getComponent(i).setEnabled(false);
						}
						else {
							getPnProducts().getComponent(i).setEnabled(true);
			...
			btnDrinks.setIcon(new ImageIcon(MainWindow.class.getResource("/img/filter/Bebidas.png")));
		...
		if (btnSides == null) {
			...
			btnSides.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					List<Product> products = mcDonalds.getMenuProducts();
					for(int i=0; i<products.size();i++) {
						if (!products.get(i).getType().equals("Side")) {
							getPnProducts().getComponent(i).setEnabled(false);
						}
						else {
							getPnProducts().getComponent(i).setEnabled(true);
			...
			btnSides.setIcon(new ImageIcon(MainWindow.class.getResource("/img/filter/Complementos.png")));
			btnSides.setMnemonic('S');
		}
		return btnSides;
	}
		...
		if (btnDesserts == null) {
			...
			btnDesserts.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					List<Product> products = mcDonalds.getMenuProducts();
					for(int i=0; i<products.size();i++) {
						if (!products.get(i).getType().equals("Dessert")) {
							getPnProducts().getComponent(i).setEnabled(false);
						}
						else {
							getPnProducts().getComponent(i).setEnabled(true);
			...
			btnDesserts.setIcon(new ImageIcon(MainWindow.class.getResource("/img/filter/Postre.png")));
			btnDesserts.setMnemonic('E');
		}
		return btnDesserts;
	}
}
